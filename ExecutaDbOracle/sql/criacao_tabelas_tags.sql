  ...
  FROM 
	(
		SELECT
			...
			avc.AVCI_CD_PROCESSO_UNICO		AS VWAT_CD_PROCESSO_UNICO	,
		...
		FROM TJRJ.TJRJ_TAG_PROCESSO_AVISO TAPR 
			INNER JOIN TJRJ_PROCESSO_AVISO TJRA  
				ON TJRA.TJRA_DK = TAPR.TGPR_TJRA_DK 
			INNER JOIN TJRJ_AVISO_COMUNIC_INTIMACAO avc
				ON avc.AVCI_TJRA_DK = TJRA.TJRA_DK
		INNER JOIN TJRJ.TJRJ_TAG_CUSTOMIZADA tag 
			ON tag.TACU_DK = TAPR.TGPR_TACU_DK 
		WHERE tag.TACU_ORGI_DK = avc.AVCI_ORGI_DK 
	)
	WHERE 1=1
	;
...
COMMENT ON COLUMN TJRJ.TJRJ_VW_AVISO_TAG.VWAT_CD_PROCESSO_UNICO IS 'Código Único do Processo. Usado na segunda instância, unicamente, para, juntamente com no Código CNJ do Processo, identificar univocamente um processo. Não é usado na 1ª instância.';
