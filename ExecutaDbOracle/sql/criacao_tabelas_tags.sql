...
( 
	...
	(
		AVCI_QT_TAGS > 0  AND 
		AVCI_CD_ID_AVISO_CANCELADO IS NULL
	)
);
...
SELECT 
	tjrj.tjrj_sq_tacu_dk.NEXTVAL AS TACU_DK ,
	SYSDATE AS TACU_DT_INCLUSAO ,
	400173 AS TACU_ORGI_DK ,
	TACU_DS_TAG ,
	TACU_COR_TAG_HEXA
FROM
(
	SELECT 'Tag 1 Aviso' AS TACU_DS_TAG, '#CC5C3B' AS TACU_COR_TAG_HEXA FROM dual UNION ALL
	SELECT 'Tag 2 Aviso' AS TACU_DS_TAG, '#526D53' AS TACU_COR_TAG_HEXA FROM dual UNION ALL
	SELECT 'Tag 3 Aviso' AS TACU_DS_TAG, '#C11742' AS TACU_COR_TAG_HEXA FROM dual UNION ALL
	SELECT 'Tag 1 Processo' AS TACU_DS_TAG, '#C9B980' AS TACU_COR_TAG_HEXA FROM dual UNION ALL
	SELECT 'Tag 2 Processo' AS TACU_DS_TAG, '#034A85' AS TACU_COR_TAG_HEXA FROM dual UNION ALL
	SELECT 'Tag 3 Processo' AS TACU_DS_TAG, '#DA7276' AS TACU_COR_TAG_HEXA FROM dual 
);

INSERT INTO TJRJ.TJRJ_TAG_AVISO
(
	TAAV_DK				,
	TAAV_TACU_DK		,
	TAAV_AVCI_DK		,
...
SELECT 
	tjrj.tjrj_sq_taav_dk.NEXTVAL 	AS TACU_DK				,
	tag.TACU_DK 					AS TAAV_TACU_DK			,		 
	avc.AVCI_DK 					AS TAAV_AVCI_DK			, 
	tag.TACU_DK						AS TAAV_ORDENACAO		,
	sysdate 						AS TAAV_DT_INCLUSAO
FROM TJRJ.TJRJ_TAG_CUSTOMIZADA tag
INNER JOIN TJRJ_AVISO_COMUNIC_INTIMACAO avc 
	ON TACU_DS_TAG LIKE ('%Aviso')
	and avc.AVCI_DK IN ( 8759418, 8759417, 8754742, 8753775, 8753727, 8747733 );

INSERT INTO TJRJ.TJRJ_TAG_PROCESSO_AVISO
(
	TGPR_DK,
	TGPR_TACU_DK,
	TGPR_TJRA_DK,
